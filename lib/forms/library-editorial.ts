...
export function applyLibraryEditorial(originals: LibraryDocument[]): LibraryDocument[] {
  return [...originals, ...NEW_DOCUMENTS, ...IMPORTED_SERVICES].map(original => {
    ...
    if (item.sourceRecordId && !extra && !item.usage) item.usage = { who: "해당 문서의 제목과 적용 조건에 해당하는 사람", when: item.description, prepare: ["본서식과 함께 필요한 증빙", "원본의 작성 방법과 해당 부표 적용 조건"], steps: ["이 문서의 제목과 원본 페이지를 확인합니다.", "해당 항목을 본서식·증빙과 대조해 사용합니다."], note: "분리한 문서는 원본의 일부입니다. 부표만으로 전체 신고·신청이 완료되지 않습니다.", timingRationale: "개별 문서의 적용 조건을 확인합니다.", sourceUrls: [item.sourceUrl] };
