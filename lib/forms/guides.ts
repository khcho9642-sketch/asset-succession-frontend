...
export const GUIDES: readonly InheritanceGuide[] = [
  {
    ...
    notice: {
      ...
      body: "가족과 재산 현황을 먼저 확인하고, 실제 계약·신청에 필요한 서식과 요건을 살펴보세요. 결정하기 어려운 사항은 상담을 통해 확인할 수 있습니다.",
    },
    steps: [
      {
        id: "inventory", title: "가족과 재산을 한눈에 정리", question: "누가 있고, 무엇을 가지고 있나요?",
        why: "가족관계, 재산, 빚과 과거 증여 이력이 이후 비교의 출발점입니다.",
        actions: ["가족 구성과 부동산·예금·주식·보험·채무 현황을 확인하세요.", "과거에 주고받은 재산과 자금이동 증빙을 모으고, 확인하지 못한 사항을 표시하세요.", "계좌·신용 조회로 거래기관을 찾고, 필요한 기준일의 잔액·부채 증빙을 개별 은행에 확인하세요."],
        resources: [
          { id: "SVC-ACCOUNTS", label: "계좌통합조회", use: "본인 계좌의 거래기관을 찾을 때" },
          { id: "SVC-CREDIT", label: "본인 신용정보 조회", use: "대출·보증 현황을 확인할 때" },
          { id: "BANK-ADD-02", label: "예금잔액증명서 발급 안내", use: "필요한 기준일의 잔액 증빙을 준비할 때" },
          { id: "BANK-ADD-03", label: "부채증명서 발급 안내", use: "차입 증빙의 종류와 기준일을 확인할 때" },
      ...
      },
      {
        ...
        actions: ["상속으로 남길지, 생전에 증여하거나 매각할지 대안을 비교하세요.", "누가 재산을 관리할지, 언제 사용할지, 가족 간 합의가 필요한 부분을 함께 정리하세요."],
        resources: [
      ...
      },
      {
        ...
        actions: ["계속 필요한 생활비·의료비와 유지할 현금, 재산에서 나오는 수입을 확인하세요.", "세금에 쓸 수 있는 자금과 마련 가능한 시기를 비교하고 부족한 부분을 상담 질문으로 남기세요."],
        resources: [
      ...
      },
      {
        ...
        actions: ["재산을 누구에게 남길지와 직접 관리하기 어려워질 때의 대응을 나눠 검토하세요.", "방법을 결정하기 전에 요건·효력·실행 절차와 맡길 사람의 역할을 확인하세요."],
        resources: [
      ...
      },
      {
        ...
        actions: ["확인한 자료와 증빙을 모으고, 실행 전에 검토받을 질문을 추리세요.", "선택한 방법에 맞는 계약·등기·세금 신고를 준비하고, 재산이나 가족 상황이 바뀌면 다시 확인하세요."],
        resources: [
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        actions: ["안심상속·상속인 금융거래조회 등 본인에게 해당하는 신청 방법을 확인하세요.", "조회 결과와 별도로 알고 있는 계약·채무 자료를 모으고, 기관별 추가 확인이 필요한 항목을 표시하세요.", "거래기관을 확인한 뒤 상속인 자격과 기준일별 잔액·부채 증빙 발급 방법을 해당 은행에 확인하세요."],
        resources: [
          { id: "P0-01", label: "안심상속 통합조회 신청서", use: "여러 기관의 재산정보를 신청할 때" },
          { id: "P0-02", label: "상속인 금융거래조회 신청서", use: "금융기관 거래를 확인할 때" },
          { id: "BANK-ADD-02", label: "예금잔액증명서 발급 안내", use: "상속인 발급 자격·방문 필요 여부를 은행에 확인할 때" },
          { id: "BANK-ADD-03", label: "부채증명서 발급 안내", use: "상속채무 증빙 발급 방법을 확인할 때" },
      ...
      },
      {
        ...
        resources: [
          ...
          { id: "BANK-ADD-01", label: "하나은행 상속예금 지급·명의변경 신청서", use: "하나은행의 신청·위임 범위를 확인한 뒤 작성" },
          { id: "P6-06", label: "퇴직연금 상속 수령 안내", use: "DB·DC·개인형IRP·기업형IRP와 계약기관을 구분" },
          { id: "BANK-ADD-06", label: "하나은행 DC·기업형IRP 지급신청서", use: "해당 제도에서 사망 시 추가 서류·사업장 확인 요건 검토" },
