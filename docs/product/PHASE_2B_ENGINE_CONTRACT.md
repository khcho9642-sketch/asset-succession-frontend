...
## 36개 내부 후보와 사용자 노출 정책

36개 시나리오는 AI가 내부적으로 검토하는 후보 라이브러리다. 사용자 화면이나 PDF에 36개 전체 목록, 전체 매트릭스 또는 제외된 시나리오를 모두 노출하지 않는다.

사용자에게 보여줄 결과는 다음으로 제한한다.

- 현 상태 그대로 상속하는 기준안 1개
- 사용자에게 가장 적합한 추천 시나리오 최대 3개
- 필요한 경우 추가 검토안 최대 2개
- 보험·연부연납 등 납세재원 보완안 1개

화면과 보고서에는 `36개 시나리오 내부 분석 완료`라는 요약 사실만 표시한다. 적용되지 않는 나머지 후보 이름과 제외 목록은 내부 테스트와 로직에서만 검증한다.
