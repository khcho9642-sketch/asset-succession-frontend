...
export function TaxComparisonEditor({ value, onChange }: Props) {
  ...
  const fields = getTaxFields(value.track).filter((field) => !["resident", "standardCase", "availableCash"].includes(field.key));
  ...
  function changeField(key: string, nextValue: string) {
    onChange({ ...value, values: { ...value.values, [key]: nextValue }, confirmed: false, confirmedAt: undefined });
  }
  ...
  function renderField(field: TaxField) {
    ...
    return (
      <div className={styles.field} key={field.key}>
        ...
        {field.type === "select" ? (
          <select id={id} data-tax-field={field.key} value={value.values[field.key] ?? ""} onChange={(event) => changeField(field.key, event.target.value)} aria-describedby={field.hint ? hintId : undefined}>
            <option value="">선택해 주세요</option>
            {field.options?.map((option) => <option value={option.value} key={option.value}>{option.label}</option>)}
          </select>
        ) : (
          <div className={styles.inputWrap}>
            <input id={id} data-tax-field={field.key} type="text" inputMode={field.type === "money" ? "decimal" : "numeric"} autoComplete="off" value={value.values[field.key] ?? ""} placeholder="직접 입력" onChange={(event) => changeField(field.key, event.target.value)} aria-describedby={field.hint ? hintId : undefined} />
  ...
  return (
    <section className={styles.editor} aria-labelledby={`${prefix}-title`} data-tax-editor>
      ...
      <div className={styles.field}>
        ...
        <p className={styles.hint}>{getTrackDescription(value.track)}</p>
      </div>
      ...
      <div className={styles.scope}>
        <label className={styles.scopeCheck} htmlFor={`${prefix}-standardCase`}>
          ...
          <span><strong>이번 계산의 적용 조건에 해당합니다.</strong><span>{getScopeStatement(value.track)}</span></span>
        </label>
        <p>해당 여부가 불확실하면 체크하지 않고 적용 조건부터 확인해 주세요.</p>
      </div>

      <div className={styles.fields}>
        {renderField({ key: "resident", label: value.track === "gift" ? "받는 사람의 국내 거주자 여부" : value.track === "business_succession" ? "증여자·수증자의 국내 거주자 여부" : value.track === "capital_gains" ? "양도자의 국내 거주자 여부" : "피상속인의 국내 거주자 여부", type: "select", options: [{ value: "yes", label: value.track === "business_succession" ? "두 사람 모두 국내 거주자" : "국내 거주자" }, { value: "no", label: value.track === "business_succession" ? "비거주자 포함" : "비거주자" }], hint: "세법상 거주자 여부를 확인해 선택해 주세요." })}
