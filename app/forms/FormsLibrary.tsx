...
export function FormsLibrary({ documents, groups, planning }: Props) {
  ...
  function cardView(card: CatalogCard) {
    const representative = card.document || card.resources.find(item => card.matchedIds.includes(item.id) && item.thumbnail) || card.resources.find(item => item.thumbnail) || card.resources[0];
    ...
    return <article className={styles.card} key={card.id} data-form-id={card.id}>
      ...
      <a className={styles.visual} href={resourceUrl(card.id)} onClick={event => openDetail(event, card.id)} aria-label={`${card.title} 자료 상세`} data-form-preview>
        {representative.thumbnail ? <Image className={styles.thumbnail} src={representative.thumbnail} width={724} height={1024} unoptimized alt={`${representative.title} 기관 제공 자료 미리보기`} /> : <span className={styles.providerVisual}>{card.group ? <FolderOpen size={33} strokeWidth={1.15} aria-hidden="true" /> : <FileText size={33} strokeWidth={1.15} aria-hidden="true" />}<span>{card.group ? "상황에 맞는 자료를 선택하세요" : representative.institution}</span></span>}
