...
def check_preservation(root, manifest, baseline, integrity):
    ...
    require(stripped == baseline, "Existing manifest fields changed; only documents[].resource may be added")
...
def validate(root: Path, manifest: dict, groups: list, baseline: dict, integrity: dict) -> dict:
    ...
    check_preservation(root, manifest, baseline, integrity)
    ...
    return {"schema_validated_resources": 177, "schema_validated_navigation_groups": 19,
            ...
            "legacy_manifest_fields_unchanged": True, "legacy_files_and_zip_unchanged": True,
