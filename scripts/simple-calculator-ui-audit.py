...
def verify_prior_conditions(page, name, width):
    ...
    choose(page, "상속 전 사전증여", "있음")
    page.get_by_role("button", name="계산하기").click()
    expect(page.get_by_label("사전증여 수증자 수", exact=True)).to_be_focused()
    page.get_by_label("사전증여 수증자 수", exact=True).fill("1")
    page.get_by_label("사전증여 1 수증자", exact=True).select_option("child")
    page.get_by_label("사전증여 1 재산 종류", exact=True).select_option("cash")
    page.get_by_label("사전증여 1 증여재산가액", exact=True).fill("100000000")
    page.get_by_label("사전증여 1 신고 과세표준", exact=True).fill("50000000")
    page.get_by_label("사전증여 1 신고 산출세액", exact=True).fill("5000000")
    page.get_by_label("사전증여 1 공제요건 확인", exact=True).select_option("unknown")
    page.get_by_role("button", name="계산하기").click()
    expect(page.get_by_label("사전증여 1 공제요건 확인", exact=True)).to_be_focused()
    expect(page.get_by_text("계산 완료", exact=True)).not_to_be_visible()
    page.get_by_label("사전증여 1 공제요건 확인", exact=True).select_option("yes")
    page.get_by_role("button", name="계산하기").click()
    expect(page.locator("aside strong").first).to_have_text("73,138,000원")
    expect(page.locator("[data-result-title]")).to_be_focused()
    expect_no_overflow(page, width)
    page.screenshot(path=str(OUT / f"{name}-prior-inheritance.png"), full_page=True)
    choose(page, "상속 전 사전증여", "없음")
    expect(page.get_by_text("다시 계산 필요")).to_be_visible()
    page.get_by_role("button", name="계산하기").click()
    expect(page.locator("aside strong").first).to_have_text("58,588,000원")
    choose(page, "상속 전 사전증여", "있음")
    expect(page.get_by_label("사전증여 1 신고 산출세액", exact=True)).to_have_value("5,000,000")
    page.get_by_label("상속개시일").fill("2026-09-19")
    page.get_by_role("button", name="계산하기").click()
    expect(page.locator("aside strong").first).to_have_text("73,138,000원")
    page.get_by_label("상속개시일").fill("2026-09-20")
    page.get_by_role("button", name="계산하기").click()
    expect(page.get_by_text("계산 완료", exact=True)).not_to_be_visible()
    expect(page.get_by_label("상속개시일")).to_be_focused()
    page.get_by_role("button", name="초기화").click()
    expect(page.get_by_label("상속개시일")).to_have_value("")

    fill_gift_base(page)
    page.get_by_label("증여재산가액").fill("150000000")
    choose(page, "그 밖의 증여에서 사용한 같은 구분의 공제", "있음")
    page.get_by_label("그 밖의 증여에서 사용한 같은 구분의 공제", exact=True).fill("50000000")
    choose(page, "혼인·출산 증여재산공제 요건 충족", "있음")
    page.get_by_label("혼인·출산 구분", exact=True).select_option("marriage")
    page.get_by_label("혼인·출산 기준일", exact=True).fill("2025-09-16")
    choose(page, "이미 사용한 혼인·출산 공제", "모름")
    page.get_by_role("button", name="계산하기").click()
    expect(page.locator('[name="marriageBirthUsedStatus"]').first).to_be_focused()
    choose(page, "이미 사용한 혼인·출산 공제", "있음")
    page.get_by_label("이미 사용한 혼인·출산 공제 금액", exact=True).fill("60000000")
    page.get_by_role("button", name="계산하기").click()
    expect(page.locator("aside strong").first).to_have_text("11,640,000원")
    expect(page.locator("[data-result-title]")).to_be_focused()
    expect_no_overflow(page, width)
    page.screenshot(path=str(OUT / f"{name}-remaining-marriage.png"), full_page=True)
    choose(page, "이미 사용한 혼인·출산 공제", "없음")
    page.get_by_role("button", name="계산하기").click()
    expect(page.locator("aside strong").first).to_have_text("4,850,000원")
    choose(page, "이미 사용한 혼인·출산 공제", "있음")
    expect(page.get_by_label("이미 사용한 혼인·출산 공제 금액", exact=True)).to_have_value("60,000,000")
    page.get_by_label("혼인·출산 기준일", exact=True).fill("2024-09-15")
    page.get_by_role("button", name="계산하기").click()
    expect(page.get_by_text("계산 완료", exact=True)).not_to_be_visible()
    expect(page.get_by_label("혼인·출산 기준일", exact=True)).to_be_focused()
    choose(page, "혼인·출산 증여재산공제 요건 충족", "없음")
    page.get_by_role("button", name="계산하기").click()
    expect(page.locator("aside strong").first).to_have_text("19,400,000원")
    page.get_by_role("button", name="초기화").click()
    expect(page.get_by_label("증여재산가액")).to_have_value("")

    fill_capital_base(page)
    choose(page, "같은 해 다른 양도소득금액", "있음")
    page.get_by_label("다른 양도소득의 구분", exact=True).select_option("profit")
    page.get_by_label("다른 양도소득금액", exact=True).fill("100000000")
    page.get_by_label("다른 양도 거래의 일반세율 확인", exact=True).select_option("yes")
    choose(page, "같은 해 양도세 납부 이력", "있음")
    page.get_by_label("같은 해 이미 납부한 양도소득세", exact=True).fill("18685000")
    choose(page, "이미 사용한 양도소득 기본공제", "있음")
    page.get_by_label("이미 사용한 기본공제", exact=True).fill("2500000")
    page.get_by_role("button", name="계산하기").click()
    expect(page.get_by_label("같은 해 이미 납부한 지방소득세", exact=True)).to_be_focused()
    page.get_by_label("같은 해 이미 납부한 지방소득세", exact=True).fill("1868500")
    page.get_by_role("button", name="계산하기").click()
    expect(page.locator("aside strong").first).to_have_text("88,852,500원")
    expect(page.locator("[data-result-title]")).to_be_focused()
    expect(page.get_by_text("추가 납부·환급 차액 (국세 + 지방소득세)", exact=True)).to_be_visible()
    expect_no_overflow(page, width)
    page.screenshot(path=str(OUT / f"{name}-annual-capital.png"), full_page=True)
    page.get_by_label("다른 양도 거래의 일반세율 확인", exact=True).select_option("no")
    page.get_by_role("button", name="계산하기").click()
    expect(page.get_by_text("계산 완료", exact=True)).not_to_be_visible()
    expect(page.get_by_label("다른 양도 거래의 일반세율 확인", exact=True)).to_be_focused()
    page.get_by_label("다른 양도 거래의 일반세율 확인", exact=True).select_option("yes")
    page.get_by_label("양도가액").fill("550000000")
    choose(page, "필요경비", "없음")
    page.get_by_role("button", name="계산하기").click()
    expect(page.locator("aside strong").first).to_have_text("-14,102,000원")
    page.screenshot(path=str(OUT / f"{name}-annual-refund.png"), full_page=True)
    page.get_by_label("양도가액").fill("900000000")
    choose(page, "필요경비", "있음")
    choose(page, "같은 해 다른 양도소득금액", "없음")
    choose(page, "이미 사용한 양도소득 기본공제", "없음")
    page.get_by_role("button", name="계산하기").click()
    expect(page.locator("aside strong").first).to_have_text("67,309,000원")
    page.get_by_role("button", name="초기화").click()
...
with sync_playwright() as p:
    ...
    for name, width, height in CASES:
        ...
        verify_prior_conditions(page, name, width)
