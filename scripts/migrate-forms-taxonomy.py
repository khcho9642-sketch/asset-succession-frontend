#!/usr/bin/env python3
"""Add resource metadata after collection; preserve frozen migration evidence.
...
--validate-existing checks a later service integration against the frozen evidence
and its explicit production overlay without rebuilding resources or baselines.
"""
...
def main():
    ...
    parser.add_argument("--validate-existing", action="store_true", help="Validate existing resources and explicit overlays without writing")
    ...
    if args.validate_existing:
        if args.apply:
            raise ValueError("--validate-existing cannot be combined with --apply")
        report = load_validator().validate(root, before, read_json(root / GROUPS),
                                          read_json(root / BASELINE), read_json(root / INTEGRITY))
        print(json.dumps({"applied": False, "validated_existing": True, **report}, ensure_ascii=False, indent=2))
        return
    if (root / BASELINE).exists() or (root / INTEGRITY).exists():
        raise ValueError("Migration baseline already exists; use --validate-existing. Preservation evidence cannot be replaced.")
