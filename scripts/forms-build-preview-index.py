...
def build(root: Path, check: bool = False, report_path: Path | None = None) -> dict:
    ...
    index = json.loads(index_path.read_text()) if index_path.exists() else {'schemaVersion': 1, 'documents': {}}
    ...
    report = {'catalogueCount': len(documents), 'ready': 0, 'provider': 0, 'pending': 0, 'archived': 0, 'items': []}
    for item in documents:
        row = {'id': item['id'], 'title': item.get('title', '')}
        if item.get('resource', {}).get('presentation', {}).get('visibility') == 'archived':
            row['status'] = 'archived'
        else:
            ...
            if previous and valid_record(item, previous, public, True):
                ...
                row['status'] = 'ready'
            elif valid_record(item, item, public, False):
                row['status'] = 'ready'
